...
def main():
    price = load_data()
    # Focused grid around the best found: SMA~30, VOL~90, LEV~2.0, TARGET~0.05
    sma_list = [20, 25, 30, 35, 40]
    vol_list = [70, 80, 90, 100, 110]
    lev_list = [1.5, 1.75, 2.0, 2.25, 2.5]
    vol_target_list = [0.03, 0.04, 0.05, 0.06, 0.07]
    results = []
    start = time.time()
    ...
    out_path = os.path.join(out_dir, f'run_focused_{timestamp}.json')
    if results:
